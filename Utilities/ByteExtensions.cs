...
public static class ByteExtensions
{
    ...
    {
        if (data is null || offset + 1 >= data.Length)
    ...
    {
        if (data is null || offset + 3 >= data.Length)
            throw new ArgumentException("Invalid offset or data length");
    ...
    {
        if (data is null || startIndex < 0 || length <= 0)
            return 0;
    ...
    {
        if (data is null || marker is null || data.Length < marker.Length)
            return false;
