...
The `FleetSummary` record provides a consolidated overview of current fleet metrics, including total device counts, online/offline status, average speed of moving vehicles, and active alert totals. It is primarily generated by `FleetDashboardService.GetFleetSummaryAsync()` to provide quick insights for monitoring and reporting.
...
public class FleetSummaryExample
{
    private readonly IFleetDashboardService _fleetDashboardService;

    public FleetSummaryExample(IFleetDashboardService fleetDashboardService)
    {
        _fleetDashboardService = fleetDashboardService;
    }

    public async Task GetFleetOverviewAsync()
    {
        // Retrieve the current fleet summary
        FleetSummary summary = await _fleetDashboardService.GetFleetSummaryAsync();

        Console.WriteLine($"Fleet Overview:");
        Console.WriteLine($"  Total Devices: {summary.TotalDevices}");
        Console.WriteLine($"  Online: {summary.OnlineDevices}");
        Console.WriteLine($"  Offline: {summary.OfflineDevices}");
        Console.WriteLine($"  Average Speed (Moving): {summary.AverageMovingSpeedKmh:F1} km/h");
        Console.WriteLine($"  Active Alerts: {summary.ActiveAlerts}");
    }
}
```
